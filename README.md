...
## 💻 Section 7: PL/pgSQL Programming — פונקציות, פרוצדורות וטריגרים (שלב ד')

בשלב זה מימשנו לוגיקה עסקית מתקדמת בתוך ה-DB באמצעות PL/pgSQL, תוך שימוש במגוון רחב של אלמנטים תכנותיים הכוללים: סמנים (**Implicit & Explicit Cursors**), החזרת **Ref Cursor**, לולאות, משפטי תנאי (**Branching**), טיפול בשגיאות (**Exceptions**) ושימוש ברשומות (**%ROWTYPE / RECORD**).

---

### 1. פונקציות (Functions)

* **`calculate_patient_bill(p_patient_id)`**
* **תיאור:** מחשבת דינמית את סך החוב של מטופל בעת שחרור, הכולל עלויות ביקור וסך מחירי התרופות שקיבל.
* **אלמנטים:** עושה שימוש ב-**Explicit Cursor** כדי לרוץ בלולאה (`LOOP`) על כל התרופות שניתנו למטופל, טוענת אותן לתוך משתנה **Record**, ומפעילה מנגנון **Exception** מותאם אישית במידה והמטופל אינו קיים.


* **`get_department_roster_cursor(p_dep_id, p_min_salary)`**
* **תיאור:** מפיקה דו"ח דינמי של צוות המחלקה המשתכר מעל סכום מסוים, ומיועדת לעבודה מול ממשק משתמש (GUI) בסריקה הדרגתית.
* **אלמנטים:** מחזירה **Ref Cursor** כמצביע לשאילתה פתוחה (מונעת העמסת זיכרון), משלבת הסתעפויות (`IF`) לבדיקות תקינות קלט עם **Implicit Cursors**.



---

### 2. פרוצדורות (Procedures - DML)

* **`apply_salary_bonus_by_performance(p_min_treatments, p_bonus_percent)`**
* **תיאור:** מתגמלת רופאים מצטיינים. מריצה סריקה על כלל הרופאים במערכת, ורופא שעבר את רף הטיפולים המוגדר מקבל תוספת שכר באחוזים.
* **אלמנטים:** שימוש נרחב ב-**Explicit Cursor** בתוך לולאה, בשילוב פקודת DML (`UPDATE`) המעדכנת את טבלת ה-`MEDICAL_STAFF`.


* **`reassign_doctor_department(p_doc_id, p_new_dep_id)`**
* **תיאור:** מטפלת בהעברת רופא ממחלקה אחת לאחרת בצורה בטוחה. מעדכנת את שיוכו ומנקה את יומן המשמרות העתידי שלו במחלקה הקודמת.
* **אלמנטים:** ביצוע מספר פקודות DML במקביל (`UPDATE` ו-`DELETE`), תוך שימוש בחריגות (`RAISE EXCEPTION`) כדי לחסום העברה של מזהה רופא או מחלקה פיקטיביים.
...
* **`trigger_audit_salary` (ON UPDATE):**
* **תיאור:** טריגר מסוג `AFTER UPDATE` המגן על טבלת `MEDICAL_STAFF`. בכל פעם שמתבצע שינוי שכר לאיש צוות, הטריגר מתעד אוטומטית את השכר הישן (`OLD.Salary`) והחדש (`NEW.Salary`) לטבלת היסטוריה חלופית (`SALARY_AUDIT`).


* **`trigger_prevent_double_shift` (ON INSERT/UPDATE):**
* **תיאור:** טריגר מסוג `BEFORE` על טבלת `SHIFT`. מונע טעויות אנוש של שיבוץ כפול באמצעות זיהוי אם איש הצוות כבר שובץ למשמרת באותו התאריך. במידה וכן, הוא עוצר את פקודת ה-DML וזורק שגיאת מערכת.
...
נוצרו שני בלוקים אנונימיים (`DO $$...BEGIN...END;$$`) המדמים תהליכי ליבה בבית החולים, כאשר כל תוכנית מזמנת פרוצדורה ופונקציה יחד:

1. **תהליך סוף חודש (End of Month Processing):** מזמנת את הפונקציה לחשב חוב למטופל, ומיד לאחר מכן מפעילה את הפרוצדורה לחלוקת בונוסי שכר לצוות הרפואי.
2. **תהליך ניוד עובדים:** קוראת לפרוצדורה `reassign_doctor_department` להעברת רופא מחלקה, ומיד לאחר מכן פותחת את ה-**Ref Cursor** בעזרת `get_department_roster_cursor` כדי להדפיס למסך בלולאה את מצבת כוח האדם המעודכנת במחלקה.
...
**דרישות מקדימות:**
- Python 3.10 ומעלה (נבדק על 3.14) עם המודול `tkinter` (מובנה בהתקנת פייתון הסטנדרטית).
- ספריית החיבור למסד הנתונים:
...
**פרטי כניסה למסך ההתחברות:**
- שם משתמש: `admin`
- סיסמה: `1234`

(מסך הכניסה גם מוודא שקיים חיבור פעיל למסד הנתונים ב-Supabase לפני המעבר לתפריט הראשי.)

## הכלים ודרך העבודה

האפליקציה נכתבה ב-**Python** עם ספריית **Tkinter** לממשק הגרפי, והתחברות
ל-**PostgreSQL** (מתארח ב-Supabase) באמצעות **psycopg2**.

הקוד בנוי בארכיטקטורת **3 שכבות** מופרדות:

| שכבה | תיקייה | תפקיד |
|------|--------|--------|
| **PL** – Presentation Layer | `stageE/PL` | מסכי Tkinter (כניסה, תפריט, CRUD, דו"חות, פעולות) |
| **BL** – Business Logic | `stageE/BL` | לוגיקה, ולידציה, מטא-דאטה של הטבלאות והמרות טיפוסים |
| **DAL** – Data Access Layer | `stageE/DAL` | חיבור למסד הנתונים והרצת SQL (פונקציות גנריות) |

### מבנה הקבצים
```
stageE/
├── mainApp.py              # נקודת כניסה - מפעיל את מסך הכניסה
├── DAL/
│   └── database.py         # חיבור + פונקציות גנריות (select/insert/update/delete/call)
├── BL/
│   ├── db_metadata.py      # תיאור 14 הטבלאות: מפתחות, עמודות, מפתחות זרים
│   ├── crud_logic.py       # לוגיקת CRUD גנרית
│   ├── reports_logic.py    # שאילתות שלב ב'
│   ├── actions_logic.py    # פונקציות ופרוצדורות שלב ד'
│   └── auth_logic.py        # אימות התחברות
└── PL/
    ├── login_window.py     # מסך כניסה
    ├── dashboard_window.py # תפריט ראשי
    ├── crud_window.py      # מסך CRUD גנרי לכל הטבלאות
    ├── reports_window.py   # מסך הרצת שאילתות
    ├── actions_window.py   # מסך פונקציות ופרוצדורות
    └── ui_common.py        # רכיבי עיצוב משותפים
```

## המסכים והפונקציונליות

### 1. מסך כניסה (Login)
נקודת הכניסה היחידה למערכת. לאחר אימות מצליח נפתח התפריט הראשי, שממנו ניתן לגשת לכל מסכי המערכת.

### 2. מסך ניהול נתונים (CRUD) – כל הטבלאות
מסך **גנרי אחד** המשרת את כל **14 הטבלאות** במערכת (נבחרת מתוך תפריט נפתח):
PERSON, MEDICAL_STAFF, DEPARTMENT, SHIFT, LAB, PATIENT, ATTENDING_DOCTOR,
MEDICATION, TREATMENT, MEDICATIONS_GIVEN, NURSE, RESEARCHER, ADDRESS, SALARY_AUDIT.

הוא תומך בכל 4 פעולות ה-CRUD:
- **שליפה (Read):** הצגת כל נתוני הטבלה ברשת.
- **הוספה (Create):** טופס דינמי המותאם לעמודות הטבלה.
- **עדכון (Update):** **המשתמש מקיש את המפתח, לוחץ "טען", והמערכת מביאה אוטומטית את שאר השדות** לעריכה (כנדרש). שדות המפתח נעולים בעת העריכה.
- **מחיקה (Delete):** לפי מפתח, עם הודעת אישור.

**הסתרת מזהים (ID) והצגת שמות:** בכל מקום שבו קיים מפתח זר, הערך מוצג כשם
ידידותי במקום מספר — גם ברשת (באמצעות JOIN) וגם בטפסים (תיבת בחירה של שמות).
לדוגמה: בטבלת `TREATMENT` מוצגים שם המטופל ושם הרופא במקום מזהי ת"ז;
בטבלת `RESEARCHER` מוצג שם המעבדה במקום `LabID`; ובטבלת `NURSE` בוחרים אדם מרשימת שמות.

**הדגמת השפעת פעולות נוספות:** עדכון שכר בטבלת `MEDICAL_STAFF` מפעיל את
הטריגר `trigger_audit_salary`, וההשפעה נראית מיד בעת שליפת טבלת `SALARY_AUDIT`
מאותו מסך. כמו כן הוספת משמרת כפולה נחסמת ע"י `trigger_prevent_double_shift`.

### 3. מסך דו"חות ושאילתות (שלב ב')
מאפשר הרצת **5 שאילתות** משלב ב' (מעל המינימום הנדרש של 2):
1. דירוג כלכליות רופאים.
2. מיטות פנויות במחלקות.
3. הכנסות מתרופות בחצי שנה.
4. חולים מבוגרים בסיכון.
5. התפלגות סוגי דם.

### 4. מסך פעולות מתקדמות (שלב ד')
מאפשר הפעלת **4 תתי-תוכניות** משלב ד' (מעל המינימום הנדרש של 2):
- **פונקציה** `calculate_patient_bill` – חישוב חשבונית מטופל.
- **פונקציה (REF CURSOR)** `get_department_roster_cursor` – צוות מחלקה לפי שכר.
- **פרוצדורה** `apply_salary_bonus_by_performance` – בונוס לרופאים מצטיינים.
- **פרוצדורה** `reassign_doctor_department` – העברת רופא בין מחלקות.

הודעות ה-`RAISE NOTICE` של הפרוצדורות נתפסות ומוצגות למשתמש בחלון פלט.

## תמונות מסך

תמונות הפעלת המערכת נמצאות בתיקייה `stageE/screenshots/`:

| תמונה | תיאור |
|-------|-------|
| `01_login.png` | מסך כניסה |
| `02_dashboard.png` | תפריט ראשי |
| `03_crud_grid.png` | מסך CRUD - הצגת טבלה עם שמות במקום מזהים |
| `04_crud_update.png` | תהליך עדכון - הקשת מפתח ושליפת השדות |
